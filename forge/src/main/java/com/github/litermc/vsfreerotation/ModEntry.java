...
import com.github.litermc.vsfreerotation.platform.ForgeConfigFile;
...
public class ModEntry {
	...
	private void onCommonSetup(final FMLCommonSetupEvent event) {
		event.enqueueWork(VSFreeRotationListeners::onModSetup);
	}
	...
	public static void onServerTick(final TickEvent.ServerTickEvent event) {
		switch (event.phase) {
		case START -> VSFreeRotationListeners.preServerTick(event.getServer());
		case END -> VSFreeRotationListeners.postServerTick(event.getServer());
		}
	}

	@SubscribeEvent
	public static void onRegisterCommands(final RegisterCommandsEvent event) {
		VSFreeRotationCommands.register(event.getDispatcher());
	}
